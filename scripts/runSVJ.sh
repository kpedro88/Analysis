...
function discrs(){
	...
	for MVA in ${BDTNAMES[@]}; do
		root -b -l -q 'KPlotDriver.C+("root://cmseos.fnal.gov//store/user/lpcsusyhad/SVJ2017/Run2ProductionV17/Skims/tree_dijetmtdetahadmf",{"input/input_svj_stack_dijetmtdetahad_2017.txt","input/input_svj_sel_bdtnew'${CONFIG}'.txt","input/input_svj_sel_bdt_incl.txt","input/input_svj_sel_full_bdt_regions.txt","input/input_svj_discr_full_bdtnew_regions.txt","input/input_svj_sets_full_bdt_regions.txt"},{"OPTION","vs:selections[pre-'${MVA}']","s+:printsuffix[_'${CONFIG}'_'${MVA}'_stack]","s:rootfile[test/bothjetAK8bdt__dijetmtdetahad_'${CONFIG}'_'${MVA}']","vs+:printformat[pdf]"},1)' >& log_discr_${MVA}_${CONFIG}.log
		# make ROC curves
