...
void plotLimit(string sname, vector<pair<string,double>> vars, vector<string> options={}){
	...
	bool do_contour = globalOpt->Get("do_contour",!acceff);
	bool do_xsec = globalOpt->Get("do_xsec",!acceff);
	...
	if(var2.empty()) {
		...
		if (labels.Has(var1)){
			...
			plotOpt->Set<int>("NdivX",-1*int(xvals.size()));
		}
	...
	//2D plot
		...
		if (labels.Has(var2)){
			...
			plotOpt->Set<int>("NdivYhisto",-1*int(yvals.size()));
		}
